...
- Entry kinds are split across screens: the Journal day-list excludes `transaction` (→ Finance) and `consumption` (→ Nutrition), showing only notes/measurements. The `/nutrition` screen surfaces the `Food` catalog + `Consumption` logging with daily `sumNutrients` totals (`oyl-nutrition*` components; `journal-store.consumptionsOn`/`dailyNutrients`; `state/foods-store.js`).
