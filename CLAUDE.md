...
- Auth routes: `/login` + `/register` (dedicated pages; login implies Remote mode; both have a "skip / use local data" option); `/profile` (identity, profile fields, connection/sync, data actions). Auth moved OUT of the Status screen. Forced login only when Remote + no session (`state/auth-guard.js`).
