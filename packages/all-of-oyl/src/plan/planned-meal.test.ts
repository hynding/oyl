...
describe('PlannedMeal', () => {
  it('is a plan due on its day, referencing a consumable with servings', () => {
    const meal = new PlannedMeal({ title: 'Oatmeal breakfast', day: day('2026-06-02'), consumable: { id: consumableId }, servings: 1.5 })
    expect(meal.kind).toBe('planned-meal')
    expect(meal.due?.value).toBe('2026-06-02')
    expect(meal.day.value).toBe('2026-06-02')
    expect(meal.consumableId).toBe(consumableId)
    expect(meal.servings).toBe(1.5)
  })

  it('defaults servings to 1 and validates', () => {
    expect(new PlannedMeal({ title: 'Oatmeal', day: day('2026-06-02'), consumable: { id: consumableId } }).servings).toBe(1)
    for (const servings of [0, -1, NaN]) {
      let caught: unknown
      try {
        new PlannedMeal({ title: 'Oatmeal', day: day('2026-06-02'), consumable: { id: consumableId }, servings })
      } catch (e) {
        caught = e
      }
  ...
  it('rejects conflicting consumable provenance', () => {
    let caught: unknown
    try {
      new PlannedMeal({ title: 'Oatmeal', day: day('2026-06-02'), consumable: { id: consumableId }, consumableId: Id.of('00000000-0000-4000-8000-000000000099') })
    } catch (e) {
      caught = e
    }
  ...
  it('round-trips JSON and can be fulfilled by a consumption', () => {
    const meal = new PlannedMeal({ id: Id.of('00000000-0000-4000-8000-000000001007'), title: 'Oatmeal breakfast', day: day('2026-06-02'), consumable: { id: consumableId } })
    ...
    expect(revived.consumableId).toBe(consumableId)
  ...
  it('throws MALFORMED_JSON on bad shapes', () => {
    for (const shape of [
      { kind: 'planned-meal', id: '00000000-0000-4000-8000-000000001007', title: 'x', status: 'open' }, // no due/consumableId/servings
      { kind: 'planned-meal', id: '00000000-0000-4000-8000-000000001007', title: 'x', status: 'open', due: '2026-06-02', servings: 1, consumableId: 'nope' },
    ]) {
      let caught: unknown
      try {
