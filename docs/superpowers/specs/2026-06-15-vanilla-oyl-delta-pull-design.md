...
- **`list`** — accept `?since`:
  ...
  const rows = await query().findMany({ where, orderBy: { updatedAt: 'asc' } })
  ```
  (R-6: verify the `$gte` operator + `orderBy` object form against a booted `db.query`; adjust if the engine spells them differently.)
...
- `src/core/http-repository.ts` `list`:
  ...
  ```
- `LocalStorageRepository` / `InMemoryRepository` / `CacheStore` **ignore `since`** (full list) — local adapters; the engine passes `since` only to the remote. Documented in each `list`.
...
- **`sync-engine.test.ts` (extend):** with a `cursors` store — first `pull()` (no cursor) full + sets the cursor to the max; add a remote record (newer `updatedAt`) → second `pull()` requests `since=cursor` and merges only the new one + advances the cursor; a pending id is skipped but the cursor still advances; `resync()` clears the cursor → the next pull is full again. (Use the protocol fake / `InMemoryRepository` as remote; the fake's `list` honors `since`? — if not, drive the cursor assertions against a small remote stub that records the `since` it was called with and filters by it.)
