...
5. `LocalStorageRepository<T>` lives in `all-of-oyl/src/core`, written against an
   injected `StorageLike`, validated by a repository **contract test suite** shared
...
- `packages/vanilla-oyl/package.json`: rename package to `@oyl/vanilla-oyl-legacy`
  (folder stays; nothing else changes there).
...
- `exports` map unchanged — only the vanilla app consumes `dist/`, by copying it,
  not via Node resolution. `dist/` is gitignored.
...
a function that, given a factory for a fresh `Repository<T>`, runs the full set of
get/list/save/delete/purge/meta/conflict assertions. Both `InMemoryRepository` and
`LocalStorageRepository` must pass it verbatim. The contract is the spec; written
...
apps/vanilla-oyl/
  index.html              # importmap { "@oyl/all-of-oyl": "./vendor/all-of-oyl/index.js" },
                          # document-level stylesheets, <oyl-shell>, module-load fallback message
  package.json            # @oyl/vanilla-oyl; devDeps only: typescript, vitest, happy-dom,
                          # http-server, @oyl/all-of-oyl (workspace:*)
  tsconfig.json           # allowJs + checkJs + strict, noEmit; '@oyl/all-of-oyl' resolves
                          # to the package's dist .d.ts (mirrors runtime resolution)
  ...
  src/
    ...
    storage/
      ...
      bootstrap.js        # constructs LocalStorageRepository per collection, hydrates
...
- `dev` — build all-of-oyl → `copy-lib` → `http-server -c-1 -p 8041`.
...
- **Import:** file picker → validate schema version (migrate if older) → run every
  shape through the revivers (`reviveEntry`/`revivePlan`/`fromJSON` — unknown kinds
  throw, so corrupt backups are rejected loudly, before anything is written) →
  replace collections atomically (all keys written only after full validation) →
  re-hydrate state. Import **replaces**; merge is explicitly out of scope.
...
- Mode defaults to `prefers-color-scheme` and live-follows the OS **until** the user
  explicitly chooses; explicit choices persist to `oyl/settings`. Three mode states:
  `system` (default), `light`, `dark`.
- Custom properties inherit through shadow boundaries — components consume tokens and
...
- **`signal(initial)`** — readable/writable reactive value; reads inside an active
  effect auto-track; writes notify dependents.
- **`computed(fn)`** — derived, lazily recomputed, trackable.
- **`effect(fn)`** — runs now, re-runs when tracked signals change; returns
  `dispose()`. Re-runs batch on a microtask: multiple writes in one tick paint once,
  and an effect never observes a half-updated batch (glitch-free).
...
batching, glitch-freedom, computed laziness, disposal, nested effects. Highest-risk
code in the app, best TDD material in it.
...
| Components | vitest + happy-dom: registration, render, bindings update, toggle interaction, disconnect cleanup (no leaked effects) |
...
(extensions, build, adapter + contract) → app skeleton (workspace, importmap,
copy-lib) → storage layer (keys, schema, bootstrap, backup, seed) → themes/tokens →
components (`OylElement`, toggle, shell) → router → Status screen → manual
acceptance pass.
